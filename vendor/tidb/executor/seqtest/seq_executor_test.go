...
func (s *seqTestSuite) TestEarlyClose(c *C) {
	...
	gofail.Enable("tidb/store/tikv/handleTaskOnceError", `return(true)`)
	defer gofail.Disable("tidb/store/tikv/handleTaskOnceError")
...
func (s *seqTestSuite) TestShow(c *C) {
	...
	row := result.Rows()[0]
	// For issue https://tidb/issues/1061
	...
	// For issue https://tidb/issues/1918
	...
	// for issue https://tidb/issues/4224
	...
	// for issue https://tidb/issues/3747
	...
	// for issue https://tidb/issues/4678
	...
	// for issue https://tidb/issues/4411
	...
	// for issue https://tidb/issues/4424
	...
	// for issue https://tidb/issues/4425
	tk.MustExec("drop table if exists show_test")
	testSQL = `create table show_test(
	...
	// for issue https://tidb/issues/4426
	tk.MustExec("drop table if exists show_test")
	testSQL = `create table show_test(
		a bit(1),
