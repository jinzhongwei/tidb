...
func (s *testSessionSuite) TestFailStatementCommit(c *C) {
	defer gofail.Disable("tidb/session/mockStmtCommitError")

	tk := testkit.NewTestKitWithInit(c, s.store)
	tk.MustExec("create table t (id int)")
	tk.MustExec("begin")
	tk.MustExec("insert into t values (1)")
	gofail.Enable("tidb/session/mockStmtCommitError", `return(true)`)
...
func (s *testSessionSuite) TestGetTSFailDirtyStateInretry(c *C) {
	defer gofail.Disable("tidb/session/mockCommitError")
	defer gofail.Disable("tidb/session/mockGetTSErrorInRetry")

	tk := testkit.NewTestKitWithInit(c, s.store)
	tk.MustExec("create table t (id int)")

	gofail.Enable("tidb/session/mockCommitError", `return(true)`)
	gofail.Enable("tidb/session/mockGetTSErrorInRetry", `return(true)`)
